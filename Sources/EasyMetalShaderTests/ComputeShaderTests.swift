...
class TestShader {
    ...
    @EMTextureArgument(.write, .type2D)
    var tex3: MTLTexture?
    
    @EMTextureArgument(.sample, .type2D)
    var tex4: MTLTexture?
    
    @EMTextureArgument(.read_write, .type2D)
    var tex5: MTLTexture?
    ...
    var impl: String {
        ...
        "float4 aa = tex1.read(gid);"
        "tex1.write(float(0), gid);"
        ...
        "float4 dd = tex4.sample(textureSampler, float2(gid.x, gid.y));"
    }
